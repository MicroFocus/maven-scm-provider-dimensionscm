...
public class DimensionsTagCmd extends AbstractTagCommand {
    ...
    private TagScmResult internalExecute(DimensionsScmProviderRepository repository, String tagName, ScmFileSet scmFileSet, ScmTagParameters scmTagParameters) throws ScmException, TransformerException, ParserConfigurationException {
        ...
        return new TagScmResult("", ok ? DimensionsConstants.COMMAND_SUCCEEDED : DimensionsConstants.COMMAND_FAILED, "ADD completed", ok);
    }

    @Override
    protected TagScmResult executeTagCommand(ScmProviderRepository repository, ScmFileSet fileSet, String tagName, ScmTagParameters scmTagParameters) {
        try {
            return internalExecute((DimensionsScmProviderRepository) repository, tagName, fileSet, scmTagParameters);
        } catch (Exception e) {

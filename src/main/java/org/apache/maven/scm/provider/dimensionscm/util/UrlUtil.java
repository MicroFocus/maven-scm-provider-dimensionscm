...
public class UrlUtil {
    ...
    private static final String SCM_CONNECTION_STR_PATTERN = "//(?:([^:]*)(?::([^@]*))?@)?([^:/]+)(?::([\\d]+|(?i)\\bcac\\b))?/([^/]+)(?:@([^/]+)?)(?:[/]?)([^/:]*)" +
            "(?:[:])([^/]*)(?:[/]?)(.*)";
    ...
    public static Matcher getMatcher(String url) throws ScmRepositoryException {
        ...
        if (!scmMatcher.find())
            throw new ScmRepositoryException("The specified url is invalid, it should use this format - " + DimensionsConstants.URL_FORMAT);
